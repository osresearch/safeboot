# The first section is processed prior to the do_mariner() API being run, the
# latter section is processed after. (This file gets included twice...)
ifneq ($(POST_MARINER),1)
...
IMAGES += simple-attest-client simple-attest-server
...
# This is the section that gets processed after the do_mariner() API is run.
# I.e. everything above has been processed by the Mariner API, which means that
# lots of autogenerated attributes are now set and can be used (such as
# <image>_<verb>_<TOUCHFILE|JOINFILE/DONEFILE>). It also means that we can no
# longer set attributes (like new IMAGES) and expect them to get processed!
