...
# Note if the caller sets MKOUT, we'll use that as our "ORIGIN", which is a
# parent makefile that includes all the child makefiles we generate, and all
# the children will be ORIGIN plus a ".x" suffix, where x=1,2,3... Also, if the
# caller sets MKOUT, we consider that a "STICKY" choice and we won't distroy
# the makefiles after generating and sourcing them.
#
# If the caller does _NOT_ set MKOUT, we;
# - won't produce a parent makefile,
# - delete each makefile after it has been finished and sourced back in (unless
#   MARINER_MKOUT_STICKY is set),
# - use "mktemp -d" to provide the storage path for this ephemeral case.
define do_mariner_prep
	...
	$(if $(MKOUT),
		...
		$(eval MARINER_MKOUT_STICKY := 1)
	,
		$(eval MARINER_MKOUT_DIR := $(shell mktemp -d))
		$(eval MARINER_MKOUT_ORIGIN := $(MARINER_MKOUT_DIR)/Makefile.out)
	)
	$(eval MARINER_MKOUT_PHONY :=)
...
define do_mariner_final
	$(eval $(call do_mariner_base))
	$(eval $(call mkout_phony))
	$(if $(MARINER_MKOUT_STICKY),,
		$(shell rm -rf $(MARINER_MKOUT_DIR) > /dev/null 2>&1))
endef
...
# already exist_!
# Also, we lazy-init when SUFFIX is empty, such that MARINER_MKOUT_ORIGIN in fact
# becomes nothing but a loader/includer of the other makefiles we produce.
define mkout_init
	$(if $(MARINER_MKOUT_SUFFIX),,
		$(if $(MARINER_MKOUT_STICKY),
			$(file >$(MARINER_MKOUT_ORIGIN),# Autogenerated by mariner.mk))
		$(eval MARINER_MKOUT_SUFFIX := 1))
	$(eval MKOUT := $(MARINER_MKOUT_ORIGIN).$(MARINER_MKOUT_SUFFIX))
	$(if $(MARINER_MKOUT_STICKY),
		$(file >>$(MARINER_MKOUT_ORIGIN),include $(MKOUT)))
...
define mkout_finish
	...
	$(eval MARINER_MKOUT_PHONY += $(filter-out $(MARINER_MKOUT_PHONY),$P))
...
# Special post-processing step that creates an extra Makefile for PHONY
# targets, if there are any.
define mkout_phony
	$(eval MARINER_MKOUT_PHONY := $(strip $(MARINER_MKOUT_PHONY)))
	$(if $(MARINER_MKOUT_PHONY),
		$(eval $(call mkout_init))
		$(eval $(call mkout_header,PHONY TARGETS))
		$(eval $(call mkout_rule,.PHONY,$(MARINER_MKOUT_PHONY)))
		$(eval $(call mkout_finish)))
endef
